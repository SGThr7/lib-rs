...
// #[cfg_attr(nightly, codesnip::entry("LSTMonoid", include("Monoid")))]
mod lst_monoid;
// #[codesnip::entry("LSTMonoid")]
...
impl<M: Monoid, LM: LSTMonoid<M>> LazySegTree<M, LM> {
    ...
    pub fn tree_index(&self) -> impl Iterator<Item = usize> {
        ...
        (0..(1 << depth))
            ...
            .map(move |i| {
                core::iter::successors(Some(i * 2), |k| Some(k * 2 + 1)).take(depth + 1 - bit(i))
            })
            .flatten()
    }
}
...
impl<M, LM> fmt::Debug for LazySegTree<M, LM>
where
    M: Monoid,
    M::Set: fmt::Debug,
    LM: LSTMonoid<M>,
...
impl<M, LM> fmt::Display for LazySegTree<M, LM>
where
    M: Monoid,
    M::Set: fmt::Display,
    LM: LSTMonoid<M>,
...
#[cfg(test)]
mod tests {
    use super::{LSTMonoid, LazySegTree, Monoid};
    use crate::math::num::alge_struct::Semigroup;
    use core::fmt::Debug;
    use std::marker::PhantomData;

    // Add monoid
    struct Mono;
    impl Semigroup for Mono {
        type Set = usize;
    ...
    // Sum act
    struct Act<M>(PhantomData<M>);
    impl<M> LSTMonoid<M> for Act<M>
    where
        M: Monoid,
    ...
    type LM = Act<Mono>;
    type LST = LazySegTree<Mono, LM>;

    #[test]
    fn from() {
        let v = vec![2, 7, 1, 8, 2, 8, 1, 8, 2, 8, 4, 6];
        let mut seg = LST::from(v.clone());
    ...
    fn operate() {
        ...
        let mut segtree = LST::new(len);
        for i in 0..len {
            for k in i..=len {
                let val = Some((k - i) % 7);
                for m in i..k {
                    ans[m] = LM::act(&ans[m], &val, 1);
                }
